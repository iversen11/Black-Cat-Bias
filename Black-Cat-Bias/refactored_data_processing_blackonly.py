...
if __name__ == '__main__': 
	...
	color_freq = (bc_data
			...
			.groupBy("Color", "MonthYear")
			.agg(count("Color").alias("color_count"))
			.orderBy("color_count", ascending = False)
			)
	...
	final_df = (count_bc
			.join(color_freq, ["Color", "MonthYear"])
			...
			.orderBy(["Color", "Year", "MonthIndex"])
			)
	yearly_df = (final_df
			.select("Color", "monthly_divBy_total", "Year")
			.groupBy("Color", "Year")
			.agg(avg("monthly_divBy_total").alias("yearly_average"))
			.orderBy(["Color", "Year"])
			)


	yearly_df.coalesce(1).write.option("header", True).csv("refactored_black_only.csv")
